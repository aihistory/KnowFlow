...
在启动或部署 KnowFlow 之前，强烈建议先运行环境检查脚本 `check_env.sh`，以确保所有依赖和目录结构都已正确配置。
...
该脚本会自动检查：
- Conda 环境及 mineru 环境是否存在
- Python 及核心依赖包
- Node.js、npm、pnpm 等前端依赖
- 项目目录结构和关键配置文件
- 主要服务端口（8888、5001、3333）是否被占用
- 前端依赖是否已安装

如有任何错误或警告，请根据提示先修复，再继续后续服务启动。
...
对于本地开发和部署，我们提供了一个功能强大的管理脚本 `start_knowflow.sh`，可以统一启动、停止、重启和查看所有服务的状态。

**服务构成:**
此脚本管理以下核心服务：
- **MinerU 服务**: 负责文档解析和处理，运行在 `8888` 端口。
- **后端服务**: 提供核心 API 接口，运行在 `5001` 端口。
- **后台管理前端**: 提供用户和系统管理界面，运行在 `3333` 端口。
...
- **启动所有服务:**
  ```bash
  ./start_knowflow.sh start
  ```

- **停止所有服务:**
  ```bash
  ./start_knowflow.sh stop
  ```
  *(该命令会强制释放可能被占用的服务端口)*

- **重启所有服务:**
  ```bash
  ./start_knowflow.sh restart
  ```

- **检查所有服务状态:**
  ```bash
  ./start_knowflow.sh status
  ```

- **查看指定服务的日志:**
  ```bash
  # 查看 MinerU 服务日志
  ./start_knowflow.sh logs mineru

  # 查看后端服务日志
  ./start_knowflow.sh logs backend

  # 查看后台管理前端日志
  ./start_knowflow.sh logs web
  ```

> 💡 **提示：** 首次运行前，请确保已根据 `start_mineru_cpu.sh` 和 `server/requirements.txt` 等文件配置好所需的 `conda` 和 `venv` 环境。
